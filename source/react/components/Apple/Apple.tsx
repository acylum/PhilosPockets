...
const Apple = (props: any) => {
  ...
  const [ref, api] = useCompoundBody(() => ({
    mass: 0.0001, position: [props.position],
    ...props,
    onCollide: (e) => onCollide(e),
    material:bouncyMaterial,
    shapes: [
      { type: 'Sphere', position: [0,0,0], rotation: [0, 0, 0], args: [0.1, 0.1, 0.1] }
    ]
  }))
  ...
  useEffect(() => {
    ...
    {
      setLastReset(props.reset);//to know when there is a value change
      api.position.copy(objState.position.x+objState.position.y+objState.position.z ? objState.position : initPos);
  ...
  return ( 
    ...
    <group ref={ref} key={18787620938}> 
    
      <mesh receiveShadow castShadow name={'apple'} 
        userData={api}
      ...
      >
        <GLTFObjDra key={187876} name="apple" receiveShadow={true} castShadow={true} position={[0,-0.095,0]} scale={[1.55,1.55,1.55]} rotation={[0,0,0]} url={'/Apple_dra.glb'} />
      </mesh>
      ...
      (
        ...
        <Points_particles position={collisionPt} name={'apple'} texture={'/points5_particle.png'}/>
        </>
      ) : (
        <>
        </>
